...
#[tauri::command]
pub async fn start_whisper_download(app: tauri::AppHandle) -> Result<(), String> {
    let dest = crate::transcription::whisper_model_path(&app)?;
    ...
    let response = client.get(WHISPER_MODEL_URL).send().await.map_err(|e| e.to_string())?;
...
pub async fn cancel_whisper_download(app: tauri::AppHandle) -> Result<(), String> {
    if let Ok(dest) = crate::transcription::whisper_model_path(&app) {
        let tmp = dest.with_extension("part");
        tokio::fs::remove_file(&tmp).await.ok();
    }
    Ok(())
