...
- **Transcript store is in-memory**: an app restart (including dev-watcher rebuilds triggered by editing src-tauri files) wipes the current/last transcript AND kills a pending save (the save thread runs 3.5 s after Stop). If a transcript "vanished", this is the likely cause. Possible future fix: journal segments to a recovery file in app data dir.
...
- **Transcript: finals are self-flushed, not recognizer-flushed** (2026-06-11): the recognizer's post-`endAudio` final is unreliable (can arrive empty/truncated — this wiped whole transcripts at gate close). Whenever a request ends (gate close, 50 s rotation, Stop), `_flushPartialAsFinal` in `capture.m` promotes the accumulated partial to a final immediately and `_suppressFinalFrom` drops the recognizer's own late final to prevent duplicates. Lanes still outlive stop() by 3 s (result handler holds the lane weakly) for cleanup; the save task waits 3.5 s after stop.
